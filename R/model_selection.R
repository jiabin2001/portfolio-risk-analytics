...
#'
#' Admission screens residual and squared-residual autocorrelation. In-sample PIT
#' KS p-values are retained as exploratory diagnostics, not used as hard admission
#' thresholds, because their null distribution ignores fitted marginal parameters.
...
select_marginal_model <- function(fits, diagnostic_alpha = 0.01, criterion = c("BIC", "AIC")) {
  ...
  rows <- lapply(seq_along(fits), function(i) {
    ...
    finite_diagnostics <- all(is.finite(c(residual_p, squared_p)))
    passed <- identical(fit$status, "ok") && diag_ok && finite_diagnostics &&
      residual_p >= diagnostic_alpha && squared_p >= diagnostic_alpha
    reason <- if (!identical(fit$status, "ok")) fit$error %||% "fit_failed" else if (!diag_ok) {
      ...
      "non_finite_diagnostics"
                   ...
                   if (squared_p < diagnostic_alpha) "remaining_arch"), collapse = ";")
    data.frame(
      ...
      pit_ks_p = pit_p, pit_diagnostic_role = "exploratory_only", diagnostics_passed = passed,
