...
#' @param allow_short Whether negative portfolio weights are permitted.
...
simulate_portfolio_risk <- function(copula_fit, marginal_fits, weights, n_simulations = 10000L,
  ...
  weights <- validate_weights(weights, 2L, allow_short = allow_short)
  ...
  with_preserved_seed(seed, {
    for (chunk in seq_along(positions)) {
      ...
      u <- simulate_copula(copula_fit, length(index), antithetic = antithetic)
  ...
  if (any(portfolio <= -1)) stop("Simulated portfolio simple return is at or below -100%.", call. = FALSE)
  portfolio_model <- if (marginal_return_type == "log") log1p(portfolio) else portfolio
  risk <- do.call(rbind, lapply(confidence_levels, function(level) empirical_var_es(portfolio_model, level)))
  list(
    portfolio_simple_returns = portfolio,
    portfolio_model_returns = portfolio_model,
    ...
    metadata = list(n_simulations = n_simulations, seed = seed, chunk_size = chunk_size,
                    ...
                    return_type = marginal_return_type,
                    marginal_models = vapply(marginal_fits, `[[`, character(1), "model_id"),
                    weights = weights, allow_short = allow_short,
...
#' @param type Quantile algorithm passed to `empirical_var_es`.
...
monte_carlo_convergence <- function(simulation_function,
                                     ...
                                     confidence_levels = c(0.95, 0.99), type = 7L) {
  rows <- list(); k <- 0L
  for (n in counts) for (seed in seeds) {
    returns <- simulation_function(n = n, seed = seed)
    risk <- risk_by_confidence(returns, confidence_levels, type)
